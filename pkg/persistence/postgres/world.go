...
func (r *WorldRepository) SaveRoomDefinition(definition *interfaces.RoomDefinition) error {
	exitsJSON, err := json.Marshal(definition.Exits)
	if err != nil {
		return fmt.Errorf("failed to marshal room exits: %w", err)
	}

	flagsJSON, err := json.Marshal(definition.Flags)
	if err != nil {
		return fmt.Errorf("failed to marshal room flags: %w", err)
	}

	query := `
		INSERT INTO room_definitions (room_id, zone_id, name, description, exits, flags)
		VALUES ($1, $2, $3, $4, $5, $6)
	...
	if _, err := r.db.Exec(query, definition.ID, definition.ZoneID, definition.Name,
		definition.Description, exitsJSON, flagsJSON); err != nil {
		return fmt.Errorf("failed to save room definition: %w", err)
	}

	return nil
}

func (r *WorldRepository) LoadRoomDefinition(roomID string) (*interfaces.RoomDefinition, error) {
	query := `
		SELECT room_id, zone_id, name, description, exits, flags
		FROM room_definitions WHERE room_id = $1`

	definition := &interfaces.RoomDefinition{}
	var exitsJSON, flagsJSON []byte
	if err := r.db.QueryRow(query, roomID).Scan(&definition.ID, &definition.ZoneID,
		&definition.Name, &definition.Description, &exitsJSON, &flagsJSON); err != nil {
		if err == sql.ErrNoRows {
			return nil, fmt.Errorf("room definition not found: %s", roomID)
		}
		return nil, fmt.Errorf("failed to load room definition: %w", err)
	}

	if err := json.Unmarshal(exitsJSON, &definition.Exits); err != nil {
		return nil, fmt.Errorf("failed to unmarshal room exits: %w", err)
	}
	if err := json.Unmarshal(flagsJSON, &definition.Flags); err != nil {
