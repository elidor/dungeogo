...
check_container_runtime() {
    if command -v docker &> /dev/null; then
        if docker compose version &> /dev/null; then
            ...
            print_status "Using runtime: docker (compose plugin)"
            return 0
        fi
        ...
        if command -v docker-compose &> /dev/null; then
            CONTAINER_CLI="docker"
            COMPOSE_CMD=(docker-compose)
            print_status "Using runtime: docker-compose"
            return 0
        fi
    fi

    # Rancher Desktop (containerd) commonly exposes nerdctl instead of docker.
    if command -v nerdctl &> /dev/null && nerdctl compose version &> /dev/null; then
        CONTAINER_CLI="nerdctl"
        COMPOSE_CMD=(nerdctl compose)
        print_status "Using runtime: nerdctl compose"
        return 0
    fi

    print_error "No compatible container runtime found. Install/configure one of:"
...
wait_for_postgres() {
    ...
    while [ $wait_time -lt $MAX_WAIT_TIME ]; do
        if "$CONTAINER_CLI" exec "$CONTAINER_NAME" pg_isready -U testuser -d postgres &> /dev/null; then
